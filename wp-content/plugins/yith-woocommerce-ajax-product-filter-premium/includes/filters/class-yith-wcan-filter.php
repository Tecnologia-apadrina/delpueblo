...
if ( ! class_exists( 'YITH_WCAN_Filter' ) ) {
	...
	abstract class YITH_WCAN_Filter {
		...
		protected function get_defaults() {
			/**
			 * APPLY_FILTERS: yith_wcan_filter_defaults
			 *
			 * Filters default parameters used for filter object initialization.
			 *
			 * @param array $defaults Default filter data.
			 *
			 * @return array
			 */
		...
		public function customize_terms( $context = 'view' ) {
			/**
			 * APPLY_FILTERS: yith_wcan_filter_customize_terms_options
			 *
			 * Filters bool representing whether current filter should use custom term options or not.
			 *
			 * @param bool             $customize_terms Should use custom terms options?
			 * @param YITH_WCAN_Filter $this            Filter object.
			 *
			 * @return bool
			 */
		 ...
		 */
		public static function get_default_term_options( $context = 'view' ) {
			$default = array(
				'label'     => '',
				'tooltip'   => '',
				'color_1'   => '',
		...
		public function get_terms_options( $context = 'view' ) {
			$term_options = $this->get_prop( 'terms', $context );

			// add information specific to edit context.
			if ( 'edit' === $context ) {
				$taxonomy     = $this->get_taxonomy( $context );
				$term_ids     = array_keys( $term_options );
				$term_options = array_filter(
					array_combine(
						$term_ids,
						array_map(
							function ( $term, $term_id ) use ( $taxonomy ) {
								...
								$term['name']    = $filter_term->name;

								if ( ! empty( $term['image'] ) ) {
									$attachment        = wp_get_attachment_image_src( (int) $term['image'], 'thumbnail' );
									$term['image_url'] = $attachment[0] ?? '';
								}
		...
		public function is_hierarchical( $context = 'view' ) {
			...
			$design       = $this->get_filter_design( $context );

			return in_array( $hierarchical, array( 'collapsed', 'expanded', 'open' ), true ) && in_array( $design, array( 'checkbox', 'radio', 'text' ), true );
		}
		...
		/**
		 * Return default price range structure
		 *
		 * @return array Default price range
		 */
		public static function get_default_price_range() {
			return array(
				'min'       => '',
				'max'       => '',
				'unlimited' => false,
		...
		public function get_additional_classes() {
			...
			if ( $this->get_preset()->should_show_filters_placeholders() ) {
				$additional_classes[] = 'filter-placeholder';
			}
			...
			/**
			 * APPLY_FILTERS: yith_wcan_filter_additional_classes
			 *
			 * Filters list of additional classes for the filter.
			 *
			 * @param array            $classes List of additional classes
			 * @param YITH_WCAN_Filter $this    Filter object.
			 *
			 * @return array
			 */
		...
		public function get_items_container_classes() {
			...
			/**
			 * APPLY_FILTERS: yith_wcan_filter_items_container_classes
			 *
		...
		public function get_data() {
			/**
			 * APPLY_FILTERS: yith_wcan_get_filter_data
			 *
			 * Filters data of current filter.
			 *
			 * @param array            $data Filter's data
			 * @param YITH_WCAN_Filter $this Filter object.
			 *
			 * @return array
			 */
		...
		protected function get_prop( $prop, $context = 'view' ) {
			...
			if ( array_key_exists( $prop, $this->data ) ) {
				...
				if ( 'view' === $context ) {
					/**
					 * APPLY_FILTERS: yith_wcan_filter_get_$prop
					 *
					 * Filters any property of the filter, when retrieved with "view" context.
					 * <code>$prop</code> is replaced with property name.
					 *
		...
		public function set_filter_design( $filter_design ) {
			if ( ! in_array( $filter_design, array_keys( YITH_WCAN_Filters_Factory::get_supported_designs() ), true ) ) {
				return;
			}
		...
		public function set_price_ranges( $ranges ) {
			...
			if ( ! empty( $ranges ) && is_array( $ranges ) ) {
				foreach ( $ranges as $range_id => $range ) {
					...
					$new_ranges[] = wp_parse_args(
						array(
							'min'       => (float) $range['min'],
							'max'       => (float) $range['max'],
							'unlimited' => isset( $range['unlimited'] ) && $range['unlimited'],
						),
						$this->get_default_price_range()
					);
				}
			}
		...
		public function set_order_options( $order_options ) {
			$supported_orders = array_keys( YITH_WCAN_Filters_Factory::get_supported_orders() );
		...
		public function render() {
			...
			// render placeholder when lazy loading filters.
			if ( $preset->should_show_filters_placeholders() ) {
				return yith_wcan_get_template( 'filters/filter-placeholder.php', $atts, false );
			}

			// check if filter is relevant for current product selection.
			if ( ! $this->is_relevant() ) {
				return '';
			}
		...
		public function render_title() {
			...
			/**
			 * APPLY_FILTERS: yith_wcan_filter_title_tag
			 *
			...
			if ( $this->is_collapsable() && ! $this->get_preset()->should_show_filters_placeholders() ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcan_filter_title_classes
			 *
			 * Filters list of classes for the filter title.
			 *
			 * @param array            $classes List of filter's title classes
			 * @param YITH_WCAN_Filter $this    Filter object.
			 *
			 * @return array
			 */
			...
			/**
			 * APPLY_FILTERS: yith_wcan_filter_title_html
			 *
			 * Filters raw HTML for the filter title.
			 *
			 * @param string           $title_html Raw html for filter title.
